...
public:
	...
	void CreateCentreline(std::string resourcesFilePath);
...
private:
	...
	void SetControlPoints(std::string resourcesFilePath);
	...
	vector<glm::vec3> m_offsetNormals;		// Offset curve points' normalized normal vectors

	unsigned int m_sampleCount;				// Number of samples to take of the splines.
	unsigned int m_vertexCount;				// Number of vertices in the track VBO
	float m_trackWidth = 350.0f;
};
